...
## Intro
# ECG QRS Detection & HRV Analysis (Research-Focused Deployment)

## 🌟 My Contributions & Project Evolution
While this project is based on the excellent implementation by Michał Sznajder, I have extended its scope to focus on **Clinical Feature Engineering**:
- **Algorithm Analysis**: Conducted a step-by-step logic audit of the Filtering, Differentiation, Squaring, and Integration stages.
- **HRV Module**: Developed a post-processing script to calculate **SDNN** and **RMSSD** metrics from the detected R-peaks.
- **Environment Engineering**: Successfully deployed the pipeline in a Python 3.12 virtual environment (`ecg_env`), resolving dependency conflicts.

---

## 🔬 Core Algorithm Understanding (Pan-Tomkins)
The QRS complex corresponds to the **ventricular depolarization** (the main pumping action) of the heart. 
This implementation follows the 5-stage pipeline:
1. **Filtering**: Isolates heart activity from measurement noise.
2. **Differentiation**: Identifies segments with high signal change.
3. **Squaring & Integration**: Makes the signal changes more distinct and smooths them for peak detection.
4. **Adaptive Thresholding**: Dynamically classifies candidates into QRS complexes or noise.

---

## 🛠 Technical Setup & Troubleshooting
- **Sampling Rate**: Tuned for 250 Hz acquisition.
- **Dependency Management**: Managed via `requirements.txt` (including `scipy`, `numpy`, `matplotlib`).
- **Debug Log**: Identified a `ValueError` in the band-pass filter stage under certain Scipy versions. This provided a deep dive into digital filter design and parameter sensitivity.
